...
const EditDietPlan = () => {
  ...
  const fetchDietPlan = useCallback(async () => {
    try {
      ...
      // Pre-populate form
      setValue('name', dietPlanData.name);
      setValue('clientId', dietPlanData.clientId?._id || dietPlanData.clientId);
      setValue('goal', dietPlanData.goal);
      setValue('dailyCalories', dietPlanData.dailyCalories);
      setValue('description', dietPlanData.description);
      setValue('restrictions', dietPlanData.restrictions);
      setValue('supplements', dietPlanData.supplements);
      setValue('hydration', dietPlanData.hydration);
      setValue('isActive', dietPlanData.isActive);
      setValue('dailyMeals', dietPlanData.dailyMeals || []);
      
      // Set selected client - handle both populated and unpopulated cases
      const client = dietPlanData.clientId || dietPlanData.client;
  ...
  const handleCaloriesCalculated = (calories) => {
    if (calories === null) {
      ...
      setShowCalculator(false);
      toast.success(`Daily calories updated to ${calories.toLocaleString()}`);
    }
  };
  ...
  if (!dietPlan) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="bg-gray-900/95 rounded-2xl shadow-xl border border-gray-800 p-8 max-w-md w-full mx-4">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-white mb-4">Diet Plan Not Found</h2>
            <p className="text-gray-400 mb-6">The diet plan you're trying to edit doesn't exist.</p>
            <button
              onClick={() => navigate('/diet-plans')}
              className="px-6 py-3 bg-gradient-to-r from-red-600 to-red-500 hover:from-red-700 hover:to-red-600 text-white rounded-xl transition-all"
            >
              Back to Diet Plans
            </button>
          </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-400">
                  Update nutrition program for {dietPlan.client?.personalInfo?.name || 'your client'}
                </p>
              </div>
            </div>
          </div>
